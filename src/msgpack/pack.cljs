...
(defn pack-float [stream f] (doto stream (stream/write-u8 0xcb) (stream/write-f64 f)))
(defn pack-int   [stream i]
  (cond
    (<=   0 i  127)               (stream/write-u8 stream i) ; +fixnum
    (<= -32 i   -1)               (stream/write-i8 stream i) ; -fixnum
    (<=   0 i 0xff)               (doto stream (stream/write-u8 0xcc) (stream/write-u8  i)) ; uint 8
    (<=   0 i 0xffff)             (doto stream (stream/write-u8 0xcd) (stream/write-u16 i)) ; uint 16
    (<=   0 i 0xffffffff)         (doto stream (stream/write-u8 0xce) (stream/write-u32 i)) ; uint 32
    (<=   0 i 0xffffffffffffffff) (doto stream (stream/write-u8 0xcf) (stream/write-i64 i)) ; uint 64
    (<= -0x80               i -1) (doto stream (stream/write-u8 0xd0) (stream/write-i8  i)) ; int 8
    (<= -0x8000             i -1) (doto stream (stream/write-u8 0xd1) (stream/write-i16 i)) ; int 16
    (<= -0x80000000         i -1) (doto stream (stream/write-u8 0xd2) (stream/write-i32 i)) ; int 32
    (<= -0x8000000000000000 i -1) (doto stream (stream/write-u8 0xd3) (stream/write-i64 i)) ; int 64
...
(extend-protocol IExtendable
  ...
  Keyword           (extension [this] (Extended. 0x03 (pack (.substring (str this) 1) (stream/output-stream))))
...
(extend-protocol Packable
  nil      (pack-bytes [_   s] (stream/write-u8 s 0xc0))
  boolean  (pack-bytes [b   s] (stream/write-u8 s (if b 0xc3 0xc2)))
...
(defn keyword-deserializer [bytes] (keyword       (unpack-stream (stream/input-stream bytes))))
(defn  symbol-deserializer [bytes] (symbol        (unpack-stream (stream/input-stream bytes))))
(defn    char-deserializer [bytes]                (unpack-stream (stream/input-stream bytes)))
(defn   ratio-deserializer [bytes] (let [[n d]    (unpack-stream (stream/input-stream bytes))] (/ n d)))
(defn     set-deserializer [bytes] (set           (unpack-stream (stream/input-stream bytes))))
(defn    date-deserializer [bytes] (let [date-str (unpack-stream (stream/input-stream bytes))] (parse-timestamp date-str)))
...
(defn unpack [buffer] (unpack-stream (stream/input-stream buffer)))
(defn pack   [data]
