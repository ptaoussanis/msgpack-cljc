...
(defn- unsigned-bytes      [bytes] (map unchecked-byte bytes))
(defn- unsigned-byte-array [bytes] (byte-array (unsigned-bytes bytes)))

(defn- fill-string [n c] (apply str (repeat n c)))
(defn- ext [byte-id ba] (i/->CustomPackable byte-id (unsigned-byte-array ba)))
...
(defn debug-round-trip [obj expected-bytes]
  (let [expected-bytes (unsigned-bytes expected-bytes)]
    ...
    (println "  expected obj:" obj)
    (println "    actual obj:" (msg/unpack expected-bytes))))
...
(deftest str-test
  ...
  (testing "str 8"
    (round-trip (fill-string 32  \b) (concat [0xd9 0x20] (repeat 32  (byte (int \b)))))
    (round-trip (fill-string 100 \c) (concat [0xd9 0x64] (repeat 100 (byte (int \c)))))
    (round-trip (fill-string 255 \d) (concat [0xd9 0xff] (repeat 255 (byte (int \d))))))

  (testing "str 16"
    (round-trip (fill-string 256   \b) (concat [0xda 0x01 0x00] (repeat 256   (byte (int \b)))))
    (round-trip (fill-string 65535 \c) (concat [0xda 0xff 0xff] (repeat 65535 (byte (int \c))))))

  (testing "str 32"
    (round-trip (fill-string 65536 \b) (concat [0xdb 0x00 0x01 0x00 0x00] (repeat 65536 (byte (int \b)))))))
...
(deftest bin-test
  ...
  (testing "bin 16" (round-trip (unsigned-byte-array (repeat 256   0x80)) (concat [0xc5 0x01 0x00]           (repeat 256   0x80))))
  (testing "bin 32" (round-trip (unsigned-byte-array (repeat 65536 0x80)) (concat [0xc6 0x00 0x01 0x00 0x00] (repeat 65536 0x80)))))

(deftest ext-test
  (testing "fixext 1"  (round-trip (ext 100 [0x80])                [0xd4 0x64 0x80]))
  (testing "fixext 2"  (round-trip (ext 100 [0x80 0x80])           [0xd5 0x64 0x80 0x80]))
  (testing "fixext 4"  (round-trip (ext 100 [0x80 0x80 0x80 0x80]) [0xd6 0x64 0x80 0x80 0x80 0x80]))
  (testing "fixext 8"  (round-trip (ext 100 [0x80 0x80 0x80 0x80 0x80 0x80 0x80 0x80]) [0xd7 0x64 0x80 0x80 0x80 0x80 0x80 0x80 0x80 0x80]))
  (testing "fixext 16" (round-trip (ext 100 (repeat 16 0x80)) (concat [0xd8 0x64] (repeat 16 0x80))))
  (testing "ext 8"
    (round-trip (ext 100 [0x1 0x3 0x11]) [0xc7 0x3 0x64 0x1 0x3 0x11])
    (round-trip (ext 100 (repeat 255 0x80)) (concat [0xc7 0xff 0x64] (repeat 255 0x80))))

  (testing "ext 16" (round-trip (ext 100 (repeat 256   0x80)) (concat [0xc8 0x01 0x00 0x64]           (repeat 256   0x80))))
  (testing "ext 32" (round-trip (ext 100 (repeat 65536 0x80)) (concat [0xc9 0x00 0x01 0x00 0x00 0x64] (repeat 65536 0x80)))))

(deftest clojure-test
  (testing "clojure.lang.Keyword"        (round-trip :abc [0xd6 0x3 0xa3 0x61 0x62 0x63]))
  (testing "clojure.lang.Symbol"         (round-trip 'abc [0xd6 0x4 0xa3 0x61 0x62 0x63]))
  (testing "java.lang.Character"         (round-trip \c   [0xd5 0x5 0xa1 0x63]))
  (testing "clojure.lang.Ratio"          (round-trip 5/2  [0xc7 0x3 0x6 0x92 0x5 0x2]))
  (testing "clojure.lang.IPersistentSet" (round-trip #{}  [0xd4 0x7 0xc0])))
...
(deftest array-test
  (testing "fixarray"
    ...
    (round-trip  [true 1 (i/->CustomPackable 100 (.getBytes "foo"))
  ...
  (testing "array 16"
    (round-trip (repeat 16    5) (concat [0xdc 0x00 0x10] (repeat 16    5)))
    (round-trip (repeat 65535 5) (concat [0xdc 0xff 0xff] (repeat 65535 5))))

  (testing "array 32"
    (round-trip (repeat 65536 5) (concat [0xdd 0x00 0x01 0x00 0x00] (repeat 65536 5)))))

(deftest map-test
  (testing "fixmap"
    (round-trip {} [0x80])
    (round-trip {1 true 2 "abc" 3 (unsigned-byte-array [0x80])} [0x83 0x01 0xc3 0x02 0xa3 0x61 0x62 0x63 0x03 0xc4 0x01 0x80])
    (round-trip {"abc" 5} [0x81 0xa3 0x61 0x62 0x63 0x05])
    (round-trip {(unsigned-byte-array [0x80]) 0xffff} [0x81 0xc4 0x01 0x80 0xcd 0xff 0xff])
    (round-trip {true nil} [0x81 0xc3 0xc0])
    (round-trip {"compact" true "schema" 0} [0x82 0xa7 0x63 0x6f 0x6d 0x70 0x61 0x63 0x74 0xc3 0xa6 0x73 0x63 0x68 0x65 0x6d 0x61 0x00])
  ...
  (testing "map 16"
    (round-trip (zipmap (range 0 16) (repeat 16 5))
      [0xde 0x0 0x10 0x0 0x5 0x7 0x5 0x1 0x5 0x4 0x5 0xf 0x5 0xd 0x5 0x6 0x5 0x3
       0x5 0xc 0x5 0x2 0x5 0xb 0x5 0x9 0x5 0x5 0x5 0xe 0x5 0xa 0x5 0x8 0x5])))

;;;; test.check

(defn- pack-and-unpack [x] (msg/unpack (msg/pack x)))
(defn- not-nan [x] (not (Double/isNaN x)))

(defspec   ints-round-trip 100 (prop/for-all [x gen/int] (= (pack-and-unpack x) x)))
...
(defspec strings-round-trip 100 (prop/for-all [x gen/string] (= (pack-and-unpack x) x)))

(def ^:private vector-of-maps (gen/vector (gen/map gen/int gen/string)))

(defspec vectors-round-trip 20 (prop/for-all [x vector-of-maps] (= (pack-and-unpack x) x)))
